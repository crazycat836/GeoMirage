...
class BookmarkImportRequest(BaseModel):
    ...
    after the migration; each row is then validated on its own by
    ``BookmarkManager.import_json`` so one bad row doesn't sink the file.
    """
    version: int = 0
...
async def import_bookmarks(req: BookmarkImportRequest):
    ...
    Rows are then validated one by one: bad rows are skipped and reported
    in ``invalid`` ({axis, index, field, reason}) while the rest import,
    and rows already in the store (same name + coordinates) are counted in
    ``skipped_duplicates`` instead of being added again. A payload the
    v0→v1 migration cannot even read is a 400 ``validation_failed``.
    """
    raw = req.model_dump()
    try:
        migrate_v0_to_v1(dict(raw))
    except Exception:
        logger.warning("Bookmark import rejected: payload failed migration", exc_info=True)
        raise http_err(
            400, ErrorCode.VALIDATION_FAILED,
            "Bookmark import payload failed validation",
        )
    bm = get_bookmark_manager()
    result = await bm.import_json(json.dumps(raw))
    return {
